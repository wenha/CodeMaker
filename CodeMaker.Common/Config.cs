            ...
            else
            {
                ...
                switch (server.Type)
                {
                    ...
                    case DatabaseType.SqlServer2008:
                        connString = string.Format("Data Source={0};Initial Catalog={1};Persist Security Info=True;User ID={2};Password={3}", server.Server, string.IsNullOrEmpty(dbName) ? string.IsNullOrEmpty(server.DatabaseName) ? GetServerDefaultDatabase(server.Type) : server.DatabaseName : dbName, server.UserID, server.Password);
                        break;
                    case DatabaseType.MySql:
                        connString = string.Format("Server={0};{1};Database={2};Uid={3};Pwd={4};", server.Server, server.Port.HasValue && server.Port.Value != -1 ? "Port=" + server.Port.Value : "", string.IsNullOrEmpty(dbName) ? string.IsNullOrEmpty(server.DatabaseName) ? GetServerDefaultDatabase(server.Type) : server.DatabaseName : dbName, server.UserID, server.Password);
                        break;
                    case DatabaseType.Sqllite:
                        connString = string.Format("Data Source={0};Pooling=true;FailIfMissing=false;{1}", server.Server, string.IsNullOrEmpty(server.Password) ? "" : string.Format("Password={0}", server.Password));
                        break;
                }
